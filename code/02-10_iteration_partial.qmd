...
for (i in 3:ncol(nematode_w)) {
  ...
  statsum <- meancv(nematode_w[[i]]) #adding two sets of square brackets converts to vector from data frame
...
nematode_mods <- nematode_w %>%
  ...
  mutate(lm = map(data,
    ~lm(resp_val ~ exp_val,
        data = .x))) %>% #.x is placeholder for whatever column placed at beginning of function
